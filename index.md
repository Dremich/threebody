...
$$
h_{t+1} = clip(RMS_{err}^{(-1/(p+1))}, 0.2, 5)
$$

### BDF Initial Steps
For all problem, I used an initial step size of $h_0 = 1\text{e-}3$. BDF5(6) additionally requires 6 initial solution points. I use adaptive RK5(4) to generate these initial conditions, as BDF5 requires a 5th order method for error convergence.
